...
abstract public class TestFunctionality {
    ...
    /**
     * Takes arguments to run the test function on an Algorithm from a class that extends the {@link TestFunctionality} Class
     * @param arr int Array that is used to run the Test
    ...
    public static Map<String, Long> test(int[] arr, int iterations, SearchObject searchObject, BiFunction<int[], SearchObject, Integer> function) {
        ...
        returnValues.put("total_time", ((totalTimeEnd - totalTimeStart)/iterations)/1000);
        returnValues.put("min_time", minTime/1000);
        returnValues.put("max_time", maxTime/1000);
        return returnValues;
    }
}